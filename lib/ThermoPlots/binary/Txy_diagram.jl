...
function _Txy_diagram(plt,model::Clapeyron.ActivityModel,p,Tmax,Tmin;iscrit=nothing,check_lle=false, check_vlle = false,lle_present=false,Npoints=200,color=:red,style=:solid)
    T_lle = []
    Klle = []
    zlle = [0.5,0.5]
    stop_vlle = false
        ...
        for l in 1:npaths
            ...
            while k <= Npoints
                ...
                # Obtain the bubble points
                bub = bubble_temperature(model,p,[x[k,l],1-x[k,l]])
                T[k,l] = bub[1]
                y[k,l] = bub[4][1]
