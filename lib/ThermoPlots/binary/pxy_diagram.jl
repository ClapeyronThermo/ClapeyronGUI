...
function _pxy_diagram!(plt,model::Clapeyron.ActivityModel,T,pmax,pmin;iscrit=nothing,check_lle=false, check_vlle = false,lle_present=false,Npoints=200,color=:red,style=:solid)
    stop_vlle = false
    p_lle = []
    Klle = []
    zlle = [0.5,0.5]
    ...
    for l in 1:npaths
        ...
        while k <= Npoints
            ...
            # Obtain the bubble points
            try
                bub = bubble_pressure(model,T,[x[k,l],1-x[k,l]])
                p[k,l] = bub[1]
                y[k,l] = bub[4][1]
                ...
                if k>2
                    ...
                    elseif check_vlle && y[k]>y[idx_vlle[1]] && !stop_vlle
                        ...
                        zlle = (x0+xx0)/2
                        zlle = [zlle, 1-zlle]
                        Klle = [xx0/x0,(1-xx0)/(1-x0)]
                        p_lle = LinRange(p0,1.5*p0,Npoints)
                        stop_vlle = true
        ...
        if lle_present
            ...
            println(zlle)
            for k in 1:Npoints
                flash = tp_flash(model,p_lle[k],T,zlle,RRTPFlash(K0=Klle,equilibrium=:lle))
                x_lle[k] = flash[1][1,1]
                xx_lle[k] = flash[1][2,1]
                Klle = flash[1][1,:]./flash[1][2,:]
                zlle = (flash[1][1,:]+flash[1][2,:])./2
